...
@loopseam Left, the cut lands where the wave crosses the center line, so the end meets the start and the seam is silent. Right, the cut lands mid-wave, so every repeat jumps to a new level — that red step is the click. Now hear the same two loops:
