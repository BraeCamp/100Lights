...
export async function POST(req: Request) {
  ...
  let body: { topic?: string; notes?: string; voice?: string }
  try { body = await req.json() } catch { return Response.json({ error: 'Invalid JSON' }, { status: 400 }) }
  const topic = body.topic?.trim()
  if (!topic) return Response.json({ error: 'Topic required' }, { status: 400 })

  // An unrecognized voice falls back to the topic-based pick rather than
  // erroring — a bad value shouldn't cost the editor a generation.
  const voice: VoiceId = body.voice && body.voice in VOICES
    ? body.voice as VoiceId
    : pickVoice(topic)
  ...
  return Response.json({ title, description, slug, body: markdown, voice })
}
