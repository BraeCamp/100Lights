...
import { getSegment, segmentUserIds } from '@/lib/saved-segments'
...
// POST /api/admin/gift/bulk — grant N days of Pro to every user in a built-in
// segment OR a saved smart segment. Capped and audit-logged.
export async function POST(req: Request) {
  if (!await isAdmin()) return Response.json({ error: 'Not signed in as admin' }, { status: 401 })
  const { segment, savedSegment, days } = await req.json().catch(() => ({})) as { segment?: string; savedSegment?: number | string; days?: number }
  const d = Number(days)
  ...
  if (savedSegment) {
    ...
    if (!seg) return Response.json({ error: 'Saved segment not found' }, { status: 404 })
    ids = await segmentUserIds(seg.criteria, CAP + 1)
    label = `saved:${seg.name}`
  } else {
  ...
  await logAdmin('gift.bulk', label, { days: d, count: target.length })
