...
import { Play, Square, Upload, Trash2, ChevronDown, ChevronRight, Star } from 'lucide-react'
import {
...
function VariationRow({ entry, onActivate, onDelete }: {
  entry: SampleEntry; onActivate: (id: string) => void; onDelete: (id: string) => void
}) {
  ...
  return (
    <div style={{
      ...
      <button onClick={() => onActivate(entry.id)} title={entry.isActive ? 'Active' : 'Set active'}
      ...
      <span style={{ flex: 1, fontSize: 10, color: entry.isActive ? 'var(--text-primary)' : 'var(--text-muted)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
        {entry.name}
      </span>
...
function TypeSlot({ type, onChanged }: { type: BeatType; onChanged: () => void }) {
  ...
  const load = useCallback(async () => {
    setVariations(await sampleGetAllByType(type).catch(() => []))
  }, [type])
  ...
  async function handleFile(file: File) {
    ...
    try {
      ...
      await load(); onChanged(); setExpanded(true)
    } finally { setUploading(false) }
  }

  async function activate(id: string) { await sampleSetActive(id, type); await load(); onChanged() }
  async function remove(id: string)   { await sampleDelete(id);          await load(); onChanged() }
    ...
    <div style={{ borderRadius: 8, border: `1px solid ${active ? 'var(--border)' : 'rgba(139,92,246,0.12)'}`, background: active ? 'var(--bg-card)' : 'rgba(139,92,246,0.03)', overflow: 'hidden' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 10px' }}>
        <button onClick={() => setExpanded(v => !v)}
          ...
          {variations.length > 0 ? (expanded ? <ChevronDown size={11} /> : <ChevronRight size={11} />) : <div style={{ width: 11 }} />}
        </button>
      ...
      {expanded && variations.length > 0 && (
        <div style={{ padding: '0 8px 8px', display: 'flex', flexDirection: 'column', gap: 4, borderTop: '1px solid var(--border)' }}>
          ...
          {variations.map(v => <VariationRow key={v.id} entry={v} onActivate={activate} onDelete={remove} />)}
        </div>
      )}
    </div>
  )
...
export default function SamplePackPanel() {
  ...
  const recount = useCallback(async () => {
    ...
    setTotalCount(new Set(all.filter(e => e.isActive).map(e => e.beatType)).size)
  }, [])
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 16 }}>
        Upload audio files for each instrument type. Click ★ on a variation to make it the active sample used by BeatLab.
      </p>
      {groups.map(group => {
        ...
        return (
          <div key={group} style={{ marginBottom: 20 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 8 }}>
              {types.map(type => (
                <TypeSlot key={`${type}-${refresh}`} type={type} onChanged={() => setRefresh(r => r + 1)} />
              ))}
            </div>
          </div>
        )
