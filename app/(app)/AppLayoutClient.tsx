...
export default function AppLayoutClient({ children }: { children: React.ReactNode }) {
  ...
  // ── One tree, three bodies ────────────────────────────────────────────────
  //
  // ⚠️ Each of these used to be its own `return`, wrapped in its own
  // <UpgradeModalProvider>. React reconciles by POSITION, so moving between the
  // editor and the rest of the app replaced one provider tree with another and
  // unmounted everything inside it.
  //
  // That was invisible while nothing inside needed to survive. Light does: it
  // is mounted below, and being unmounted on every navigation is precisely the
  // bug this change exists to fix. Building the body and wrapping it once keeps
  // Light in a stable position across every trip.
  const editorBody = <div className="h-full">{children}</div>
