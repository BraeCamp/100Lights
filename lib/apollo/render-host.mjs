...
//
// The '@/' path alias is opaque to Node, so any module using it is loaded from
// a temp copy with the alias rewritten. This used to special-case presets.ts
// and broke the day patch.ts itself grew an alias import — so the rewrite is
// generic, and there is exactly ONE instance of patch.ts so presets.ts shares
// its initPatch/uid rather than getting a second.
...
async function loadAliased(rel, { header = '', substitutions = [] } = {}) {
  if (!tmpDir) {
    tmpDir = mkdtempSync(path.join(tmpdir(), 'apollo-host-'))
...
/** patch.ts, tables.ts and presets.ts, loaded once per process. */
export function apolloModules() {
  modulesPromise ??= (async () => {
    ...
    const tablesMod = await loadAliased('lib/apollo/tables.ts')
    const presetsMod = await loadAliased('lib/apollo/presets.ts', {
    ...
    return {
      ...
      FACTORY_PRESETS: presetsMod.FACTORY_PRESETS,
    }
  })()
 ...
 */
export function loadEngineClass() {
 ...
 */
export async function createRenderHost({ patch, bpm, playing = false, seed = 1 } = {}) {
  const [Cls, mods] = await Promise.all([loadEngineClass(), apolloModules()])
