...
export const LEARN_ENTRIES: LearnEntry[] = [
  ...
  E('ems-ladder', 'EMS Ladder filter', 'A wild diode ladder — asymmetric, unstable, characterful.',
    'Modeled after the diode ladders in old EMS synths: every stage saturates asymmetrically, and high [[resonance]] folds back into the signal instead of staying polite. Great for aggressive, unstable, slightly broken tones.', ['ems ladder']),
  E('mg-dirty', 'MG Dirty filter', 'A ladder with heavy, asymmetric input drive.',
    'The same 4-pole [[ladder]] core driven HARD: the input stage clips asymmetrically before the poles, so even quiet sounds pick up grit and harmonics. Use it when a sound needs attitude more than accuracy — the Drive knob goes much further here.', ['mg dirty']),
  E('comb2', 'Comb 2 filter', 'A damped feedback comb — smoother, more "tuned" than Comb ±.',
    'A comb filter whose feedback loop passes through a lowpass damper, so the metallic ring decays naturally like a plucked string instead of buzzing forever. Cutoff tunes the comb pitch, [[resonance]] sets the feedback, and the third knob darkens the damping.', ['comb 2']),
  E('exp-bpf', 'Exp BPF filter', 'A steep, exponentially-resonant bandpass.',
    'Two bandpass stages in series with a tight resonance law: at low [[resonance]] it isolates a band cleanly; near the top the emphasis grows exponentially into a whistling, almost sine-pure peak. Sweep it for vocal, resonant leads.', ['exp bpf']),
  E('fx-racks', 'FX Racks', 'Whole-chain presets — save, load, copy and paste entire effect lanes.',
    'A rack is the entire effect chain of one lane saved as a unit. Racks ▾ offers factory racks (curated chains like Echo Band Distortion or Big Retro Mod Reverb), your own saved racks, and lane operations: Copy lane / Paste lane moves a whole chain between Main and the busses, Clear empties it. Save a rack when a chain sounds good on anything.', ['racks', 'racks ▾', 'rack']),
  E('fx-gate', 'Gate', 'Silences the signal when it falls below a threshold.',
    'A noise gate (downward expander): when the level drops under Thresh, the gain glides down by Range dB — cutting hiss, bleed and tails between notes. Attack is how fast it reopens, Hold keeps it open briefly after the signal dips, Release is the fade-down speed.', ['gate', 'noisegate']),
  E('fx-deesser', 'De-esser', 'Tames harsh "s" and cymbal sizzle dynamically.',
    'A de-esser watches one high band (Freq/Width) and turns ONLY that band down when it gets hot — sibilance ducks, the rest of the tone stays. Thresh sets where it starts acting, Amount caps how hard it cuts.', ['de-esser', 'deesser']),
  E('fx-transients', 'Transient shaper', 'Reshapes the hit vs the body of a sound.',
    'Splits every sound into its transient (the hit) and its sustain (the body) by comparing a fast and a slow envelope. Attack boosts or softens the hit; Sustain fattens or dries the body — drum punch, plucky keys, roomy tails, all from two knobs.', ['transients', 'transient shaper']),
  E('fx-dyneq', 'Dynamic EQ', 'An EQ band that only acts when its frequency gets loud.',
    'One peak band (Freq/Q) whose gain follows the band\'s own level: past Thresh it moves toward Range (cut when negative, boost when positive). Surgical resonance control that leaves quiet moments untouched.', ['dyn eq', 'dyneq', 'dynamic eq']),
  E('fx-autopan', 'Auto-Pan', 'An LFO that moves the sound between the speakers.',
    'Pans the signal left-right on a cycle (Rate, Depth, Wave). Phase sets the offset between channels: 180° is classic auto-pan, 0° makes both channels pulse together — which is tremolo.', ['auto-pan', 'autopan', 'tremolo']),
  E('pz-filter', 'PZ (Pole-Zero) filter', 'Design the filter by placing its poles and zeros.',
    'A direct pole-zero filter: Cutoff sets the POLE angle (where the response peaks), [[resonance|Res]] its radius (how sharply it rings), Fat sets the ZERO angle (where the response dips to nothing) and Drive its depth. Poles boost, zeros notch — combining them builds shapes ordinary filters can\'t.', ['pz', 'pole-zero', 'pz (pole-zero)']),
  E('key-range', 'Key range', 'This oscillator only plays inside a keyboard range.',
    'Keys Lo/Hi map an [[oscillator]] to part of the keyboard (0–127 MIDI notes). Give osc A the low half and osc B the top half for a split keyboard, or overlap ranges so a layer only joins above a certain note — Serum calls this OSC mapping.', ['keys', 'key range']),
