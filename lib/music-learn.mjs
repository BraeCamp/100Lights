...
// Shaping + audio→MIDI hybrid — bundled from lib/*.ts via scripts/build-analyzers.mjs.
import { analyzeShaping, shapeToRollFx } from '../scripts/analyzers/audio-shape.mjs'
import { audioToNotes } from '../scripts/analyzers/audio-to-midi.mjs'
...
function stemFeatures(name, signal, sr, ctx) {
  ...
  // Shaping: the FX + articulation the corpus was missing (filter/brightness sweep, vibrato, tremolo,
  // drive, reverb, slide) → mapped to the app's roll-fx vocabulary so the capture is reproducible
  // through the program. APPROXIMATE, like the rest of the reverse-engineering.
  feat.shape = analyzeShaping(signal, sr)
  // Pitch-based FX are meaningless on unpitched percussion (the "vibrato/tremolo" there is just the
  // hit periodicity, which the drum grid already captures) — null them for drum stems.
  if (role === 'drums') { feat.shape.vibrato = null; feat.shape.tremolo = null; feat.shape.slide = 0 }
  const fx = shapeToRollFx(feat.shape)
  feat.rollFx = fx.rollFx
  if (fx.articulation) feat.articulation = fx.articulation
...
export async function analyzeSong({ stems, mixPath, opts = {} } = {}) {
  ...
  // ── Note/chord UPGRADE: replace the crude per-onset f0 / chroma with our audio→MIDI hybrid — real
  // polyphonic notes incl. locally-detected chords — for every pitched (non-drum, active) stem.
  for (let i = 0; i < stemFeats.length; i++) {
    const f = stemFeats[i]
    if (!f.active || f.role === 'drums') continue
    const { notes, chordsResolved } = await audioToNotes(loaded[i].data, sr, {})
    f.notes = notes.map(n => ({
      startSec: round(n.startSec, 3), durSec: round(n.durSec, 3),
      midi: n.midi, note: NOTE_NAMES[((n.midi % 12) + 12) % 12],
      velocity: n.velocity <= 1 ? Math.max(1, Math.round(n.velocity * 127)) : Math.round(n.velocity),
    }))
    f.noteCount = f.notes.length
    f.chordsDetected = chordsResolved
  }
  ...
  return {
    ...
    confidenceNotes: {
      ...
      shaping: 'APPROXIMATE — FX/articulation estimated from audio (filter/vibrato/tremolo/drive/reverb/slide → roll-fx)',
    },
  }
}

// ── Corpus recording ───────────────────────────────────────────────────────────
// On-disk corpus directory names. NOT lib/slugify.ts: keeps underscores and caps
// at 48, and this is an .mjs Node script that cannot import the .ts module.
// Existing ~/100lights-ml-corpus entries are named with these rules — changing
// them orphans the corpus.
function corpusSlug(s) {
...
\`{ id, ts, prompt, params, bpm, key, mode, progression, stems:[{role,loudnessDb,onsetDensity,shape,rollFx}], sections, densityArc, spectralBalance, summary }\`
\`shape\` = reverse-engineered FX/articulation per stem: \`{ brightnessHz, filterMotion, vibrato, tremolo, drive, reverb, slide }\`; \`rollFx\` = same mapped to the app's roll-fx params (reproducible).
...
// Build an EDITABLE .cfproj from the analysis — a note clip + roll-fx per pitched stem, so the capture
// can be opened and edited IN the program (per the "songs must be editable through the program" rule),
// not just stored as raw feature numbers. Notes come from the audio→MIDI hybrid.
export function buildCfproj(analysis) {
  const tempo = analysis.tempo.bpm || 120
  const tracks = [], arrangementClips = []
  for (const s of analysis.stems) {
    if (!s.active || !s.notes || !s.notes.length) continue
    const trackId = corpusSlug(s.name)
    tracks.push({ id: trackId, name: s.name, role: s.role, rollFx: s.rollFx || {}, articulation: s.articulation || null })
    arrangementClips.push({
      trackId, startBeat: 0, rollFx: s.rollFx || {},
      notes: s.notes.map(n => ({
        pitch: n.midi,
        startBeat: round((n.startSec * tempo) / 60, 3),
        durationBeats: round(Math.max(0.0625, (n.durSec * tempo) / 60), 3),
        velocity: n.velocity,
      })),
    })
  }
  return {
    _type: '100lights-capture', schemaVersion: 1,
    note: 'Reverse-engineered from ElevenLabs audio (notes + roll-fx). Editable in the program; ElevenLabs internals NOT captured.',
    dawProject: { tempo, timeSig: [4, 4], tracks, arrangementClips },
  }
}
...
export function recordToCorpus({ title, prompt, params = {}, analysis, mixPath, stemPaths = [], model = 'music_v2', requestInfo = null } = {}) {
  ...
  // analysis.json + an editable capture.cfproj (note clips + roll-fx, reproducible in the program)
  writeFileSync(join(dir, 'analysis.json'), JSON.stringify(analysis, null, 2))
  try { writeFileSync(join(dir, 'capture.cfproj'), JSON.stringify(buildCfproj(analysis), null, 2)) } catch { /* best-effort */ }
  ...
  for (const entry of stemPaths) {
    ...
    const label = typeof entry === 'object' && entry.name ? corpusSlug(entry.name) : basename(p, '.wav')
    let fname = `${label}.wav`
  ...
  const line = {
    ...
    stems: analysis.stems.map(s => ({ role: s.role, loudnessDb: s.loudnessDb, onsetDensity: s.onsets.densityPerSec, shape: s.shape || null, rollFx: s.rollFx || null })),
