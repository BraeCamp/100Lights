...
export const COMMAND_SUMMARIES: Record<string, string> = {
  ...
  envelope_shape: 'A known shape into an automation lane — sine, triangle, saw, inverse saw, square, a ramp either way, or an ADSR across the span — or Simplify, which takes the points back out again. Drawing a clean four-bar sine by hand is impossible and a rough one is pointless; a recorded gesture arrives as sixty points that read as a line. It lands on the song loop when there is one and the whole lane otherwise, and everything outside that span is left alone. The sine it inserts is the same one the LFOs make.',
