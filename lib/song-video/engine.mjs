...
export function mountSongVideo(canvas, song, options = {}) {
  ...
  function fireOnsets(a, b) { for (const n of song.notes) if (a < b ? (n.s > a && n.s <= b) : (n.s > a || n.s <= b)) { if (!o.media) synth(n); if (fmt.onHit) fmt.onHit(n, f) } }
  ...
  function drawChrome() {
    ...
    ctx.fillStyle = '#eceafd'; ctx.font = `800 ${Math.round(W * 0.046)}px system-ui`; ctx.textAlign = 'left'; ctx.fillText(o.brand, pad + 22, H * 0.075)
    if (o.meta) { ctx.fillStyle = '#8b88a8'; ctx.font = `600 ${Math.round(W * 0.026)}px ui-monospace, monospace`; ctx.fillText(o.meta, pad + 1, H * 0.108) }
    ctx.textAlign = 'center'
    o.hook.slice(0, 2).forEach((ln, i) => { ctx.fillStyle = ln.accent ? hexa(o.accent, 0.95) : '#eceafd'; ctx.font = `800 ${Math.round(W * 0.062)}px system-ui`; ctx.fillText(ln.text, W / 2, H * (0.865 + i * 0.04)) })
  ...
  function loop() {
    ...
    const bg = ctx.createLinearGradient(0, 0, 0, f.H); bg.addColorStop(0, '#0a0912'); bg.addColorStop(1, '#050409'); ctx.fillStyle = bg; ctx.fillRect(0, 0, f.W, f.H)
    fmt.draw(f)
    drawChrome()
