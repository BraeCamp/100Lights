...
import { drawBackground, drawCover, drawKeyframes } from './backgrounds.mjs'
...
const DEFAULTS = {
  ...
  // Cheap-AI background layer: a free audio-reactive procedural style ('aurora'|'nebula'|'waves', or
  // 'none' for the plain gradient), or a pooled/cached AI image (bgImage) reused across songs.
  bgStyle: 'none', bgImage: null, bgImages: null,   // bgImages = keyframe sequence → cheap AI video
  // Hook-text styling: colour, position (top|center|bottom), entry animation
  // (none|fade|rise|pop), and an outline for legibility over busy visuals.
  textColor: '#eceafd', hookPos: 'bottom', hookAnim: 'none', textOutline: false,
  // Extra free-form text layers: { text, x, y (0..1), size (rel), color, align,
  // from, to (beats — omit for always-on) }. Enables lyric/timed + placed text.
  layers: [],
  // Fixed render resolution for export quality (else sizes to the element).
  width: 0, height: 0,
  // An AnalyserNode tapping the real audio — audio-reactive formats read
  // f.freq / f.wave from it each frame (null → those stay null, note-driven only).
  analyser: null,
}
...
export function mountSongVideo(canvas, song, options = {}) {
  ...
  const f = { ctx, W: 0, H: 0, beat: 0, pulse: 0, SPB, LOOP, now: 0, hexa, rr, accent: o.accent, tracks: song.tracks, fieldTop: 0, fieldBot: 0,
    freq: null, wave: null, pmin, pmax,
    px: p => (0.08 + (p - pmin) / (pmax - pmin) * 0.84) * f.W }
  // Reusable analysis buffers (filled from o.analyser each frame when present).
  let freqArr = null, waveArr = null

  function resize() {
    // Fixed render resolution (export quality) wins; otherwise size to the element.
  ...
  function drumHit(n, t0, tvol) {
    const vel = (0.4 + 0.6 * (n.v / 100)) * (typeof tvol === 'number' ? tvol : 1), p = n.p
  ...
  function synth(n) {
    ...
    const tk = song.tracks[n.tr] || {}
    if (tk.muted) return
    const tvol = typeof tk.vol === 'number' ? tk.vol : 1
    const kind = tk.kind || 'melodic'
    if (kind === 'drum') { drumHit(n, t0, tvol); return }
    const v = VOICE[kind] || VOICE.melodic
    const freq = 440 * Math.pow(2, (n.p - 69) / 12)
    const peak = v.gain * (0.4 + 0.6 * (n.v / 100)) * tvol, dur = Math.min(n.d * SPB, v.cap)
  ...
  let playing = false, startPerf = 0, lastBeat = 0, raf = 0, justStarted = false
  ...
  function drawChrome() {
    ...
    // Hook text — coloured, positioned, optionally animated + outlined.
    const hookLines = o.hook.slice(0, 2).filter(ln => ln && ln.text)
    if (hookLines.length) {
      ctx.textAlign = 'center'
      const baseY = o.hookPos === 'top' ? 0.15 : o.hookPos === 'center' ? 0.5 : 0.865
      const p = Math.min(1, f.beat / 2) // entry progress over the first 2 beats of each loop
      let alpha = 1, dy = 0, scale = 1
      if (o.hookAnim === 'fade') alpha = p
      else if (o.hookAnim === 'rise') { alpha = p; dy = (1 - p) * -H * 0.045 }
      else if (o.hookAnim === 'pop') scale = 1 + 0.07 * pulse
      hookLines.forEach((ln, i) => {
        const size = Math.round(W * 0.062 * ts * scale)
        ctx.font = `800 ${size}px ${o.font}`
        const x = W / 2, y = H * (baseY + i * 0.045) + dy
        if (o.textOutline) { ctx.lineWidth = size * 0.13; ctx.strokeStyle = `rgba(0,0,0,${0.55 * alpha})`; ctx.lineJoin = 'round'; ctx.strokeText(ln.text, x, y) }
        ctx.fillStyle = ln.accent ? hexa(o.accent, 0.95 * alpha) : hexa(o.textColor, alpha)
        ctx.fillText(ln.text, x, y)
      })
    }
    // Free-form text layers (placed + optionally time-gated for lyrics).
    if (o.layers) for (const L of o.layers) {
      if (!L || !L.text) continue
      if (L.from != null && f.beat < L.from) continue
      if (L.to != null && f.beat > L.to) continue
      const size = Math.round(W * ((L.size || 0.05)) * ts)
      ctx.textAlign = L.align || 'center'
      ctx.font = `800 ${size}px ${o.font}`
      const x = W * (L.x == null ? 0.5 : L.x), y = H * (L.y == null ? 0.5 : L.y)
      if (o.textOutline) { ctx.lineWidth = size * 0.13; ctx.strokeStyle = 'rgba(0,0,0,0.55)'; ctx.lineJoin = 'round'; ctx.strokeText(L.text, x, y) }
      ctx.fillStyle = hexa(L.color || o.textColor, 1); ctx.fillText(L.text, x, y)
    }
  ...
  function loop() {
    ...
    // First frame after play(): just sync the clock, don't fire. The <audio>
    // element can still report its previous currentTime for a frame, which would
    // otherwise fire EVERY onset from 0→that beat at once (all notes flash at the
    // start, out of sync with the audio).
    if (playing) {
      if (justStarted) justStarted = false
      else if (f.beat >= lastBeat) fireOnsets(lastBeat, f.beat)
      else { fireOnsets(lastBeat, LOOP); fireOnsets(0, f.beat) }
    }
    lastBeat = f.beat
    ctx.globalCompositeOperation = 'source-over' // formats may set 'lighter'; never let it leak between frames
    ctx.clearRect(0, 0, f.W, f.H)
    // bgStyle 'video' = leave the canvas transparent so a real <video> behind it shows through (the note
    // overlay use case). Any other style paints the gradient base first.
    if (o.bgStyle !== 'video') { const bg = ctx.createLinearGradient(0, 0, 0, f.H); bg.addColorStop(0, (o.bg && o.bg[0]) || '#0a0912'); bg.addColorStop(1, (o.bg && o.bg[1]) || '#050409'); ctx.fillStyle = bg; ctx.fillRect(0, 0, f.W, f.H) }
    if (o.analyser) {
      if (!freqArr) { freqArr = new Uint8Array(o.analyser.frequencyBinCount); waveArr = new Uint8Array(o.analyser.fftSize) }
      o.analyser.getByteFrequencyData(freqArr); o.analyser.getByteTimeDomainData(waveArr)
      f.freq = freqArr; f.wave = waveArr
    }
    // Cheap-AI background layer: keyframe "video" (N cached AI stills → motion) > single pooled image >
    // free audio-reactive procedural. All $0 per frame or amortized per video.
    const bt = o.media ? o.media.currentTime : (performance.now() - startPerf) / 1000
    if (o.bgImages && o.bgImages.length) drawKeyframes(ctx, o.bgImages, f.W, f.H, bt, o, f)
    else if (o.bgImage) drawCover(ctx, o.bgImage, f.W, f.H)
    else if (o.bgStyle && o.bgStyle !== 'none' && o.bgStyle !== 'video') drawBackground(ctx, o.bgStyle, o, f.W, f.H, bt, f)
    fmt.draw(f)
    drawChrome()
  ...
  return {
    canvas,
    play() { initAudio(); if (AC && AC.resume) AC.resume(); if (o.media) { o.media.currentTime = 0; o.media.play().catch(() => {}) } playing = true; startPerf = performance.now(); lastBeat = 0; justStarted = true },
