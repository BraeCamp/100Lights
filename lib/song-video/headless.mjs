...
export async function recordFormatVideo(browser, { wavBuf, songData, format, meta, hook, seconds, root, tmpDir, accent = '#a78bfa', dropBurst = false }) {
...
const inst=mountSongVideo(document.getElementById('c'),SONG,{format:${aj(format)},brand:'100LIGHTS',meta:${aj(meta)},hook:${aj(hook || [])},accent:${aj(accent)},loopBeats:SONG.loopBeats,synth:false,media,analyser});
