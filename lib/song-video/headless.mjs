...
export async function recordFormatVideo(browser, { wavBuf, songData, format, meta, hook, seconds, root, tmpDir, accent = '#a78bfa', dropBurst = false, bare = false, bgStyle = 'none', bg = null }) {
...
const inst=mountSongVideo(document.getElementById('c'),SONG,{format:${aj(format)},brand:${aj(bare ? '' : '100LIGHTS')},meta:${aj(bare ? '' : meta)},hook:${aj(bare ? [] : (hook || []))},accent:${aj(accent)},bgStyle:${aj(bgStyle)},bg:${aj(bg)},loopBeats:SONG.loopBeats,synth:false,media,analyser});
