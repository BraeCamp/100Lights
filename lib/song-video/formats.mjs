...
// ── AUDIO-REACTIVE formats ──────────────────────────────────────────────────
// These read f.freq (byte FFT) / f.wave (byte time-domain) from the real audio's
// analyser. When there's no analyser (paused preview) they fall back to a gentle
// beat-driven idle so the format still reads.

// Log-ish band magnitude 0..1 from the spectrum for bar index i of N.
const band = (freq, i, N) => {
  if (!freq) return 0
  const lo = Math.floor(Math.pow(i / N, 1.7) * freq.length * 0.55) + 2
  const hi = Math.max(lo + 1, Math.floor(Math.pow((i + 1) / N, 1.7) * freq.length * 0.55) + 2)
  let m = 0; for (let b = lo; b < hi && b < freq.length; b++) m = Math.max(m, freq[b])
  return m / 255
}

// 7 · EQ BARS — a frequency-bar spectrum along the bottom, reacting to the mix.
function eqBars(song, opts) {
  const N = 44
  return { draw(f) {
    const { ctx, W, H } = f, y0 = f.fieldBot, maxH = (f.fieldBot - f.fieldTop) * 0.94
    const bw = (W * 0.88) / N, x0 = W * 0.06
    for (let i = 0; i < N; i++) {
      const mag = f.freq ? band(f.freq, i, N) : 0.06 + 0.05 * (1 + Math.sin(f.beat * 3 + i * 0.5))
      const h = Math.max(3, Math.pow(mag, 0.8) * maxH), x = x0 + i * bw
      const grad = ctx.createLinearGradient(0, y0 - h, 0, y0)
      grad.addColorStop(0, f.hexa(f.accent, 0.98)); grad.addColorStop(1, f.hexa(f.accent, 0.28))
      ctx.fillStyle = grad; f.rr(x + bw * 0.12, y0 - h, bw * 0.76, h, Math.min(bw * 0.32, 5)); ctx.fill()
      ctx.fillStyle = f.hexa('#ffffff', 0.35 + 0.45 * mag); f.rr(x + bw * 0.12, y0 - h, bw * 0.76, 3, 2); ctx.fill()
    }
  } }
}

// 8 · RADIAL SPECTRUM — the spectrum wrapped into a ring around a pulsing core.
function radialSpectrum(song, opts) {
  const N = 72
  return { draw(f) {
    const { ctx, W, H } = f, cx = W / 2, cy = H * 0.47, r0 = Math.min(W, H) * 0.15, R = Math.min(W, H) * 0.3
    ctx.save(); ctx.translate(cx, cy); ctx.globalCompositeOperation = 'lighter'
    for (let i = 0; i < N; i++) {
      const mag = f.freq ? band(f.freq, i, N) : 0.12 + 0.08 * (1 + Math.sin(f.beat * 3 + i * 0.4))
      const ang = (i / N) * Math.PI * 2 - Math.PI / 2, len = R * (0.12 + Math.pow(mag, 0.85))
      const c = Math.cos(ang), s = Math.sin(ang)
      ctx.strokeStyle = f.hexa(f.accent, 0.28 + 0.6 * mag); ctx.lineWidth = (W / N) * 0.55
      ctx.beginPath(); ctx.moveTo(c * r0, s * r0); ctx.lineTo(c * (r0 + len), s * (r0 + len)); ctx.stroke()
    }
    ctx.globalCompositeOperation = 'source-over'
    ctx.strokeStyle = f.hexa(f.accent, 0.55 + 0.4 * f.pulse); ctx.lineWidth = 2
    ctx.beginPath(); ctx.arc(0, 0, r0 * (1 + 0.04 * f.pulse), 0, 7); ctx.stroke()
    ctx.restore()
  } }
}

// 9 · WAVEFORM — the live oscilloscope of the mix, mirrored + glowing.
function waveform(song, opts) {
  return { draw(f) {
    const { ctx, W, H } = f, mid = H * 0.47, amp = H * 0.24, w = f.wave
    ctx.save(); ctx.globalCompositeOperation = 'lighter'
    const trace = (lw, a) => {
      ctx.strokeStyle = f.hexa(f.accent, a); ctx.lineWidth = lw; ctx.beginPath()
      if (w) { const step = Math.max(1, Math.floor(w.length / W))
        for (let x = 0, i = 0; x < W; x++, i += step) { const y = mid + ((w[Math.min(i, w.length - 1)] - 128) / 128) * amp; x ? ctx.lineTo(x, y) : ctx.moveTo(x, y) } }
      else { for (let x = 0; x < W; x++) { const y = mid + Math.sin(x * 0.025 + f.beat * 4) * amp * 0.28; x ? ctx.lineTo(x, y) : ctx.moveTo(x, y) } }
      ctx.stroke()
    }
    trace(9, 0.18); trace(2.5, 0.95)
    ctx.restore()
  } }
}

// 10 · PIANO KEYS — a keyboard along the bottom; keys light in their track colour
// as their notes hit (note-driven).
function pianoKeys(song) {
  const lit = Object.create(null) // pitch -> {beat, tr}
  const isBlack = p => [1, 3, 6, 8, 10].includes(((p % 12) + 12) % 12)
  return {
    onHit(n, f) { lit[n.p] = { beat: f.beat, tr: n.tr } },
    draw(f) {
      const { ctx, W, H } = f
      const lo = Math.max(21, Math.floor(f.pmin) - 1), hi = Math.min(108, Math.ceil(f.pmax) + 1)
      const whites = []; for (let p = lo; p <= hi; p++) if (!isBlack(p)) whites.push(p)
      const kw = W / Math.max(1, whites.length), kbTop = H * 0.6, kbH = H * 0.3
      const glow = p => { const l = lit[p]; if (!l) return 0; const dt = ((f.beat - l.beat) % f.LOOP + f.LOOP) % f.LOOP; return Math.max(0, 1 - dt / 0.55) }
      // white keys
      whites.forEach((p, i) => {
        const x = i * kw, g = glow(p), c = col(f, lit[p]?.tr ?? 0)
        ctx.fillStyle = g > 0 ? f.hexa(c, 0.35 + 0.6 * g) : 'rgba(230,228,245,0.9)'
        f.rr(x + 1, kbTop, kw - 2, kbH, 3); ctx.fill()
        if (g > 0) { ctx.save(); ctx.globalCompositeOperation = 'lighter'; ctx.shadowColor = f.hexa(c, 0.9); ctx.shadowBlur = 22 * g; f.rr(x + 1, kbTop, kw - 2, kbH, 3); ctx.fillStyle = f.hexa(c, 0.5 * g); ctx.fill(); ctx.restore() }
      })
      // black keys (between whites)
      const bw = kw * 0.62, bh = kbH * 0.62
      whites.forEach((p, i) => {
        const bp = p + 1
        if (bp > hi || !isBlack(bp)) return
        const x = i * kw + kw - bw / 2, g = glow(bp), c = col(f, lit[bp]?.tr ?? 0)
        ctx.fillStyle = g > 0 ? f.hexa(c, 0.6 + 0.4 * g) : '#141220'
        f.rr(x, kbTop, bw, bh, 2); ctx.fill()
      })
    },
  }
}
...
export const FORMATS = {
  ...
  'eq-bars': { name: 'EQ bars', create: eqBars },
  'radial': { name: 'Radial spectrum', create: radialSpectrum },
  'waveform': { name: 'Waveform', create: waveform },
  'piano': { name: 'Piano keys', create: pianoKeys },
