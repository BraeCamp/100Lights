...
const SONG = ['Pad', 'Bass', 'Drums', 'Hats', 'Lead']
...
// ── the round trip ─────────────────────────────────────────────────────────
{
  ...
  check('and comes back for the same sentence said differently',
    got?.calls.length === 1 && got.calls[0].name === 'set_track' && got.calls[0].input.target === 'pad',
    JSON.stringify(got))
  check('an exact hit says so, so the caller knows what it may forget', got?.from === 'exact')

  // The caller hands these to a planner that may keep them.
  got.calls[0].input.target = 'bass'
  check('what comes back is a copy, not the stored entry',
    recallCommand('mute the pad')?.calls[0].input.target === 'pad')
}
...
// ── what must never be learned ─────────────────────────────────────────────
{
  ...
  // Accepted either way: a spoken number is also a slot, so this stores the
  // sentence AND "set the tempo to {n0}".
  check('a number they DID say is fine',
    rememberCommand('set the tempo to 120',
      [{ name: 'set_tempo', input: { bpm: 120 } }]).startsWith('stored'))
...
// ── one lesson, a whole family ─────────────────────────────────────────────
//
// ⚠️ Remembering exact sentences barely helps: "mute the pad" teaches nothing
// about "mute the bass", so a ten-track song would need ten paid lessons for one
// idea. The arguments become slots, and only the arguments.
{
  forgetLearned()
  const r = rememberCommand('mute the pad',
    [{ name: 'set_track', input: { target: 'pad', mute: true } }], SONG)
  check('a lesson with a track name in it also stores a template',
    r === 'stored-with-template', r)

  const other = recallCommand('mute the bass')
  check('so a track never taught is answered for free',
    other?.calls[0].input.target === 'bass' && other.from === 'template',
    JSON.stringify(other))
  check('and the taught sentence still takes the exact path',
    recallCommand('mute the pad')?.from === 'exact')

  // The number is an argument too.
  forgetLearned()
  rememberCommand('set reverb to 100% on the pad',
    [{ name: 'set_effect', input: { target: 'pad', effect: 'reverb', amount: 100 } }], SONG)
  const n = recallCommand('set reverb to 40% on the drums')
  check('a percentage generalises with the track',
    n?.calls[0].input.amount === 40 && n.calls[0].input.target === 'drums',
    JSON.stringify(n?.calls))
  check('but the effect name stays literal, so delay is not answered by reverb',
    !recallCommand('set delay to 40% on the drums'))
}

// ── what must not generalise ───────────────────────────────────────────────
{
  forgetLearned()
  // ⚠️ "{0}" alone would match every sentence ever spoken.
  rememberCommand('pad', [{ name: 'select_track', input: { target: 'pad' } }], SONG)
  check('a sentence that is nothing but its argument makes no template',
    !recallCommand('bass'), JSON.stringify(recallCommand('bass')))

  // A name that is not in the song is not an argument — it is just a word.
  forgetLearned()
  const r = rememberCommand('turn on the metronome',
    [{ name: 'set_metronome', input: { on: true } }], SONG)
  check('a lesson with no arguments stores only the exact sentence', r === 'stored')
  check('and it still answers its own sentence', recallCommand('turn on the metronome')?.from === 'exact')
}
