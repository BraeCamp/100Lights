  ...
  return (
    <div style={{ flexShrink: 0, borderTop: '1px solid var(--border)', background: 'var(--bg-surface)' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: landscape ? 6 : 8, padding: landscape ? '3px 10px' : '9px 12px' }}>
        ...
        <button onClick={() => setFxOpen(o => !o)} aria-pressed={fxOpen} aria-label="Performance FX" title="Hold-to-play FX" style={{ ...tBtn, width: 34, fontSize: 15, color: fxOpen ? 'var(--accent-light)' : 'var(--text-muted)', borderColor: fxOpen ? 'var(--accent)' : 'var(--border)', background: fxOpen ? 'rgba(139,92,246,0.14)' : 'var(--bg-card)' }}>🎛️</button>
        <button
