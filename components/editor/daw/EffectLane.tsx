...
function EffectRow({
  ...
  effects, isLast,
  onEditTarget, onCtxMenu, onShapeTarget,
}: {
  rowIndex: number
  trackId: string
  beatW: number
    ...
    >
      ...
      <div style={{ position: 'absolute', top: 0, bottom: 0, left: -scrollLeft, width: (viewEndBeat + 10) * beatW }}>
        {rowEffects.map(eff => {
          ...
          return (
            <div key={eff.id}
              style={{ position: 'absolute', left, width, top: 3, bottom: 3, background: `${color}30`, border: `1px solid ${color}`, borderRadius: 3, overflow: 'hidden', cursor: 'grab', userSelect: 'none' }}
              onMouseDown={e => {
              ...
              onClick={e => { e.stopPropagation(); onEditTarget({ effect: eff, x: e.clientX, y: e.clientY }) }}
