...
const FEATURES: Feature[] = [
  ...
  { group: 'Voice', name: 'Asking for a Sound in Your Own Words', helpIds: [], modes: ['music'], hint: 'Say it to Light — "make the pad fuzzier", "let\'s make it wiggle".',
    description: 'You do not have to know what a low-pass is. Describe the sound: fuzzier, wiggly, dreamy, bigger, harder, muffled, crunchy, echoey, roomy. Where a word means two different things here, Light asks which — "do you mean more like static, or more muffled?" — and describes both for somebody who has not heard the words before. Then it makes the change for real, plays it back where it happens with a couple of bars of run-up, and asks how it sounds. From there it stays on the table: "a little bit less of that", "way more", "start that way then come down" (which spreads it across the bars), "undo that", or "that\'s good". Those follow-ups only mean anything while something is under discussion, which is exactly when you would say them.' },
  { group: 'Arrangement Tools', name: 'Time Commands', helpIds: [], modes: ['music'], hint: 'Set the song loop over the span, then ⌘K → "insert silence", "delete time" or "duplicate time".',
    description: 'A span of the whole arrangement opens up, closes up, or happens twice — and everything moves with it: the clips on every track, the automation points, the cue markers, and the tempo and time-signature changes. A time command that moved the clips and left the automation behind would look right for about a bar and then be quietly wrong for the rest of the song. A clip the silence lands inside is stretched rather than shoved forward, and a clip that only overlaps a deleted span is trimmed rather than deleted. The span is the song loop when one is set, otherwise a bar at the playhead.' },
  { group: 'Arrangement Tools', name: 'Insert Shape and Simplify', helpIds: ['insert-shape', 'simplify-envelope'], modes: ['music'], hint: 'The ∿ menu and the ⌁ button on an automation lane\'s header. Or type "insert shape" in ⌘K.',
    description: 'Two opposite halves of one problem. Drawing a clean four-bar sine by hand is impossible and drawing a rough one is pointless, so Insert Shape puts a known one in: sine, triangle, saw, inverse saw, square, ramps, and an ADSR across the whole span. It lands on the song loop when there is one and the whole lane otherwise, and everything outside that span is left exactly alone — shaping one section is the commonest reason to reach for it. A square gets two points at every edge so the edge is actually vertical; one point would make it a triangle. And the sine it inserts is the same sine the LFOs make, from the same function, because two answers to what a sine is shows up as a sound nobody can account for. Simplify goes the other way: a recorded gesture arrives as sixty points that read as a line, which is worth having in the take and unbearable to edit, so this finds the fewest points that still draw the same shape. The ends never move — that would change the sound rather than tidy the picture.' },
  { group: 'Arrangement Tools', name: 'Renaming a Run of Tracks', helpIds: [], modes: ['music'], hint: 'Select a track and press ⌘R, then Tab down the list.',
    description: 'Naming eight tracks by hand is eight double-clicks, eight select-alls, eight types and eight clicks away — about forty actions for what is really one. ⌘R opens the selected track\'s name; Tab commits it and opens the next one. A hash anywhere in the name is the auto-number: type "Gtr #" and Tab four times and you get Gtr 1, Gtr 2, Gtr 3, Gtr 4. Several hashes pad, so "Gtr ##" gives 01 and 02, for a set that will be sorted as text somewhere else. Deliberately explicit rather than clever — noticing a trailing number and incrementing it reads well until somebody renames a track "Take 2" and finds the next one called "Take 3" without asking. Tab past the last track stops rather than wrapping, because a run that wrapped would quietly overwrite the names you just typed.' },
  { group: 'Arrangement Tools', name: 'Undo History', helpIds: ['undo-history', 'history-row'], modes: ['music'], hint: 'Press ⌘⌥Z, or type "undo history" in ⌘K.',
    description: 'Everything you have done, and a way back to any of it. Undo on its own is a door you can only walk through one step at a time, in the dark: you press it, look, press it again, look. The history is the same stack with the lights on, so you can see that the edit you want gone was four requests ago and go there in one click. One row per REQUEST, not per action — a spoken command that adds a filter, automates it and moves the playhead is three entries on the stack and one thing that happened, and listing all three would just be a worse version of pressing undo three times. What you have already undone stays above a "you are here" line, greyed, and clicking it walks forward again. The words are the same ones the voice transcript uses for that edit, so the two never describe one change two ways.' },
  { group: 'Arrangement Tools', name: 'Bounce', helpIds: ['bounce-track'], modes: ['music'], hint: 'Right-click a track header → Bounce to New Track, or press ⌘B with a track selected.',
    description: 'A track\'s sound printed as audio. Bouncing looks like freezing and is not: a freeze is a cache that thaws back, and a bounce is a normal audio clip you can cut, warp, reverse and drag about. Freezing is about CPU; bouncing is about committing to a sound so the next decision is made on top of it rather than beside it. To New Track puts the audio on a new track beside the source and parks the originals rather than deleting them, so you can hear the difference and bring them back with the clip activator. In Place replaces the track\'s own clips. The render is pre-mixer — the track plays at unity and centre with its sends off, so what is printed is its devices and nothing else — and then the level, the pan and the sends are copied onto the new track, so the bounce sounds identical to the source and the level is still in the fader where you can move it. Live leaves the new track at default instead, which means a bounce of a track you had pulled down comes back twice as loud. The result goes into the Sound Library rather than a temporary URL, so it survives a reload. A track that prints silence is refused rather than landing a silent clip that looks exactly like a working one.' },
  { group: 'Recording', name: 'The Click', helpIds: ['metronome', 'metronome-settings', 'metronome-sound', 'metronome-rhythm', 'metronome-only-recording', 'count-in'], modes: ['music'], hint: 'Right-click the metronome button in the transport bar. Or type "metronome sound" in ⌘K.',
    description: 'A click you cannot hear over what you are playing is worse than no click at all — you drift, and you do not know you are drifting. So it has six sounds, chosen to sit in different frequency bands rather than to have different characters, because cutting through is a question of which band is free: Click is a short high ping, Beep is the most audible and the least pleasant, Stick is broadband and survives a busy mix, Wood is easy on the ear over long sessions, Cowbell sits above a kit without fighting the hats, and Rimshot is a crack with body under it. The rhythm decides how often: Auto subdivides below 90 BPM, where the beat is further apart than the phrase you are trying to place inside it, and thins out to once a bar above 200, where it would be a buzz. Only While Recording keeps playback clean and puts the click there for the take. The count-in shows as negative bars ticking down — counting up from zero would put 1.1.1 on screen before the song has started, which is the same number you see once you are already late.' },
  { group: 'Recording', name: 'Record Quantization', helpIds: ['record-quantize', 'pad-record-quantize'], modes: ['music'], hint: 'Press Record — "Quantize as played" sits in the record setup box. Or type "record quantization" in ⌘K.',
    description: 'Notes land on the grid as you play them, so a part you cannot quite play in time arrives in time and you keep going instead of stopping to tidy up. This is not the same as quantizing afterwards: that is an edit you can see, weigh and undo, and this happens at the moment of capture. Only note starts move — the lengths are kept exactly as held, because length is the part of a performance that still reads as playing once the timing has been taken away. The 1/8-and-1/8T grids snap to whichever line is nearer, for a phrase that moves between straight and triplet feel. Nothing is ever dropped: a grid coarser than what you played stacks notes on one line, and the studio says so rather than quietly deleting the second one.' },
  { group: 'Recording', name: 'Punch In and Punch Out', helpIds: ['punch-in', 'punch-out'], modes: ['music'], hint: 'The two arrow buttons beside Loop in the transport bar (⌥R and ⇧⌥R).',
    description: 'Fixing one phrase in the middle of a take without risking what is either side of it. Drag the loop brace over the part you want to replace, switch on Punch In, and start a few bars early: you play along, and the recorder starts by itself exactly at the brace. Punch Out stops it at the end of the brace. Use them together and the take is exactly that span, hands free. The punch region is the loop brace itself rather than a second pair of markers to drag, so it is the same "this part of the song" you set everywhere else, and it works whether or not Loop is switched on. Pressing record with the playhead already inside the brace starts the take now instead of waiting for a pass that would throw away what you just played, and a playhead past the brace is refused out loud rather than recording silence.' },
  { group: 'Mixer', name: 'Crossfader Curves', helpIds: ['crossfader', 'crossfader-curve'], modes: ['music'], hint: 'The mixer row under the arrangement (⌘⌥M) → Crossfader.',
    description: 'Assign a track to side A or side B and one control fades between them. The curve decides how. Equal power is the everyday one: two sources at the centre are as loud as one at an end. Linear adds the levels instead of the power, so the middle sounds louder — which is right when both sides are the same recording. Slow fade keeps more of both across the travel, fast cut drops the outgoing side away quickly, and hard cut is a switch rather than a fade. They are named for what they do rather than after another program\'s list.' },
  { group: 'Session', name: 'Playing the Grid from the Keyboard', helpIds: ['session-grid'], modes: ['music'], hint: 'Click the session grid once, then use the arrows.',
    description: 'A performance does not have a spare hand for a mouse. Click into the grid and the arrows move a highlight around it: Enter launches the clip under it, Shift+Enter launches the whole scene, Control+Enter stops that track and Shift+Control+Enter stops everything. Page Up and Page Down jump eight scenes. The highlight stops at the edges rather than wrapping, so Down at the bottom cannot quietly point at the top of the set and fire the wrong clip on the next press. ⌘I inserts a scene above it, and ⇧⌘I captures whatever is playing right now into a new scene below — a set you found by hand, kept.' },
  { group: 'Recording', name: 'Automation Recording', helpIds: ['automation-arm'], modes: ['music'], hint: 'The Auto menu in the transport bar, then record and move a control.',
    description: 'Moving a control while the transport is recording writes the move into its lane. Drawing automation is precise and slow; recording it is imprecise and fast, and for anything that is a performance — a filter opening across a build, a fader riding a vocal — the fast imprecise one is right, because a gesture does not survive being typed in. ⚠️ Touch and Latch are not two flavours of one thing. Touch writes only while you hold the control, and letting go returns the lane to what it already said: that is for fixing one moment in a shape you like. Latch writes while you hold and then KEEPS the last value to the end of the song, replacing everything after it — which is also how you wipe eight bars of careful work by touching a knob at bar one, so it is never the default and it says how many points it destroyed. Off is the third real answer: moving a control overrides its lane instead, which is the existing behaviour and destroys nothing. A control you have never automated still records — the lane is made for you, with the units the parameter actually uses. And recording into an overridden lane switches it back on, because recording is you telling the lane what to say, the opposite gesture to taking manual control.' },
  { group: 'Recording', name: 'Nudge', helpIds: ['nudge-earlier', 'nudge-later'], modes: ['music'], hint: 'The two small arrows in the transport bar, live only. Or type "nudge" in ⌘K.',
    description: 'Slides the whole song a few milliseconds earlier or later against the wall clock, without changing the tempo or where the playhead is. It is for playing along with something the studio cannot hear — a record, another machine, somebody in the room. You are on the right tempo and the wrong phase, and no amount of tempo correction fixes phase: you have to push the whole thing sideways. Nothing in the project changes, so there is nothing to undo and nothing to save.' },
  { group: 'Session', name: 'Global Quantization', helpIds: ['global-quantization'], modes: ['music'], hint: 'The small menu beside Loop in the transport bar, or ⌥6 to ⌥0.',
    description: 'When a session clip\'s launch lands, for every slot that does not name its own. None fires the instant you press; 1 Beat, 1 Bar, 2 Bars and 4 Bars wait for the next line, so a clip you press half a bar early still comes in on time. It sits on the control bar and has keys because it is the one setting you change while playing: a set that has been landing on the bar wants to go to a beat for the fill and back afterwards. ⚠️ The slot menu has offered "Use Global" from the very start, and there was no global — every such slot got a hard-coded bar from the engine, and nothing anywhere could change it. Not on Live\'s ⌘6 to ⌘0, because ⌘0 is "interface back to 100%" here and in every browser this studio runs in; taking it would break a reflex nobody would think to look up.' },
  { group: 'Session', name: 'Track Status', helpIds: ['track-status'], modes: ['music'], hint: 'Launch a session clip — the line appears at the top of that track\'s header.',
    description: 'What each track\'s session is doing: the clip that is sounding, and — when it has a follow action — how many beats are left before that action fires. The number counts down as it plays, so you can see a chain coming before you hear it. Nothing playing, nothing said.' },
  { group: 'Session', name: 'Follow Actions', helpIds: ['follow-actions', 'follow-a', 'follow-chance'], modes: ['music'], hint: 'Right-click a session slot — Follow Action is under the launch settings.',
    description: 'What a clip does when its turn is over: stop, play again, move to the next or previous clip in the column, the first or last, any of them, any OTHER one, or jump to a named scene. Give it a second action and a chance, and a column of clips shuffles itself — three parts to one means the first happens three times as often. It fires after the clip\'s own length (Linked), or after a time you set. Empty slots are skipped and the ends wrap round, so a chain of four set to Next keeps going. ⚠️ These used to fire only when a clip stopped on its own, which a looping clip never does — so they did nothing on the normal case, and stopped entirely when you left the session view. The engine runs them now.' },
